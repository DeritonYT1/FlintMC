...
public class DefaultPropertyMapSerializer implements PropertyMap.Serializer {
  ...
  @Inject
  public DefaultPropertyMapSerializer(Property.Factory propertyFactory, PropertyMap.Factory propertyMapFactory) {
  ...
  public PropertyMap deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
    PropertyMap result = this.propertyMapFactory.create(new HashMap<>());
        ...
        if (entry.getValue() instanceof JsonArray) {
          for (JsonElement element : ((JsonArray) entry.getValue())) {
            result.put(entry.getKey(),
                    this.propertyFactory.create(entry.getKey(), element.getAsString())
            );
          }
        }
      }
